(function () {
  ...
  const logaritmoln = (a) => {
    return a <= 0 ? mostrarError() : Math.log(a);
  };

  const raiz = (a) => {
    return a <= 0 ? mostrarError() : Math.sqrt(a);
  };
  ...
  const logaritmo = (a) => {
    return a <= 0 ? mostrarError() : Math.log10(a);
  };

  const hipotenusa = (...args) => Math.hypot(args);
  const botonesGenericos = document.querySelectorAll(
    ".calculadora .botones .filas"
  );
  ...
  const notacionCientifica = (patron1, callback, input, func = true) => {
    ...
    while (coincidenciasRealizadas < problemaExpresion.length) {
      let exponente = null;
      if (!func) {
        exponente = problemaExpresion[coincidenciasRealizadas].match(
          /\(-{0,1}\d+(\.\d+)*\)/
        );
  ...
  const comprobarPatrones = () => {
    ...
    if (/{|\[|\[/gi.test(expresion.value)) {
      expresion.value = expresion.value.replace(/\{|\[/gi, "(");
    }
    ...
    if (/cos\(-{0,1}\d+(\.\d+)*\)/gi.test(expresion.value)) {
    ...
    if (/sin\(-{0,1}\d+(\.\d+)*\)/gi.test(expresion.value)) {
    ...
    if (/tan\(-{0,1}\d+(\.\d+)*\)/gi.test(expresion.value)) {
    ...
    if (/ln\(-{0,1}\d+(\.\d+)*\)/gi.test(expresion.value)) {
    ...
    if (/log\(-{0,1}\d+(\.\d+)*\)/gi.test(expresion.value)) {
    ...
    if (/√\(-{0,1}\d+(\.\d+)*\)/gi.test(expresion.value)) {
    ...
    if (/-{0,1}\d+\^\(2\)/gi.test(expresion.value)) {
      console.log("expresion a^(2) detectada");
      notacionCientifica(/-{0,1}\d+\^\(2\)/gi, elevado, expresion);
    }

    // compruebo si existe a^(±b)
    if (/-{0,1}\d+\^\(-{0,1}\d+(\.\d+)*\)/gi.test(expresion.value)) {
      console.log("expresion a^(b) detectada");
      notacionCientifica(
        /-{0,1}\d+\^\(-{0,1}\d+(\.{0,1}\d+)*\)/gi,
        elevado,
        expresion,
        false
      );
    ...
    if ((_expresion = expresion.value.match(/\|-{0,1}\d+(\.{0,1}\d+)*\|/gi))) {
      ...
      for (const producto of _expresion) {
        ...
        expresion.value = expresion.value.replace(
          `${producto}`,
          Math.abs(reemplazar)
        );
      }
    ...
    /* comprobar una multiplicacion por parentesis (NO SON ACEPTADOS NUMEROS NEGATIVOS) ejemplo:
            ...
            ...)23.1  --> ...)*23.1 
        */
    if ((_expresion = expresion.value.match(/\)\d+(\.{0,1}\d+)*/gi))) {
      console.log("expresion de producto ...)n detectada", _expresion);
    ...
    /* comprobamos si existe un producto no declarado por un * ejemplo:
        ...
        */
    if (
      (_expresion = expresion.value.match(
        /(\(-{0,1}\d+(\.\d+)*\)\(-{0,1}\d+(\.\d+)*\))+/gi
      ))
    ) {
  ...
  for (const fila of botonesGenericos) {
    fila.addEventListener("click", (e) => {
      ...
      if (errorSintaxis) {
        ...
        if (tipo == "operacion" && operacion == "=" && expresion.value) {
          try {
            comprobarPatrones();
            ...
            document.getElementById(
              "resultado"
            ).innerHTML = `<span class='error' style='display:block; text-align:rigth'>
                                                                                Verifique la expresión.
                                                                            </span>`;
          }
        } else if (tipo == "numero") {
          if (errorSintaxis) limpiarPantalla();
          expresion.value += operacion;
        } else if (tipo == "operacion" && operacion == "+") {
          expresion.value += "+";
        } else if (tipo == "operacion" && operacion == "-") {
          expresion.value += "-";
        } else if (tipo == "operacion" && operacion == "x") {
          expresion.value += "*";
        } else if (tipo == "operacion" && operacion == "÷") {
          expresion.value += "/";
        } else if (tipo == "funcion" && operacion == "DEL") {
          if (expresion.value.length > 0) {
            expresion.value = expresion.value.substring(
              0,
              expresion.value.length - 1
            );
          }
        } else if (tipo == "funcion" && operacion == "AC") {
          limpiarPantalla();
        } else if (tipo == "operacion" && operacion == "(") {
          expresion.value += "(";
        } else if (tipo == "operacion" && operacion == ")") {
          expresion.value += ")";
        } else if (tipo == "operacion" && operacion == "x10x") {
          expresion.value += "*10^()";
        } else if (tipo == "funcion" && operacion == "a/b") {
          expresion.value += "a/b";
        } else if (tipo == "funcion" && operacion == "√") {
          expresion.value += "√()";
        } else if (tipo == "funcion" && operacion == "x2") {
          expresion.value += "a^(2)";
        } else if (tipo == "funcion" && operacion == "x□") {
          expresion.value += "a^(b)";
        } else if (tipo == "operacion" && operacion == ".") {
          expresion.value += ".";
        } else if (tipo == "funcion" && operacion == "log") {
          expresion.value += "log()";
        } else if (tipo == "funcion" && operacion == "ln") {
          expresion.value += "ln()";
        } else if (tipo == "funcion" && operacion == "tan") {
          expresion.value += "tan()";
        } else if (tipo == "funcion" && operacion == "sin") {
          expresion.value += "sin()";
        } else if (tipo == "funcion" && operacion == "cos") {
          expresion.value += "cos()";
        } else if (tipo == "funcion" && operacion == "|x|") {
          expresion.value += "|x|";
        }
